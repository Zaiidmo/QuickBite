...
@section('content')
    <section class="lg:mx-auto overflow-hidden mx-4 lg:pr-16 grid grid-cols-1 md:grid-cols-2 ">
        <div class="col-span-1 md:col-span-2">
            ...
            {{-- Modal To Update a User's Role --}}

            @foreach ($users as $user)
                <section id="updateUserRole-{{ $user->id }}" class=" relative hidden">
                    <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 overflow-auto">
                        <div
                            class="absolute bg-black/50 backdrop-blur-xl border-2 border-primary rounded-lg overflow-scroll shadow">
                            <button data-user-id="{{ $user->id }}" id="modalCloser-{{ $user->id }}"
                                onclick="closeUpdatePopup({{ $user->id }})" type="button"
                                class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center popup-close">
                                <svg aria-hidden="true" class="w-5 h-5" fill="#c6c7c7" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd"
                            ...
                            <div class="p-5">
                                ...
                                <form class="mx-8 4 lg:mx-0 font-poppins font-semibold tracking-wide"
                                    action="{{ route('permissions.update', $user) }}" method="POST"
                                    enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')

                                    <div class="w-full">
                                        <label for="role_id" class="block text-sm text-gray-300">Roles</label>
                                        <select multiple placeholder="Choose Roles" name="role_id[]" id="roles"
                                            class="users_permissions block text-xs mt-2 w-full placeholder-gray-300 rounded-lg border border-gray-200 bg-secondary/60 px-2 py-2.5 text-gray-300 focus:border-blue-300 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40 ">
                                    ...
                                    <div class="w-full">
                                        <label for="userPermissions" class="block text-sm text-gray-300">Permissions</label>
                                        <select multiple placeholder="Choose Permissions" name="userPermissions[]"
                                            id="users_permissions"
                                            class="users_permissions block text-xs mt-2 w-full placeholder-gray-300 rounded-lg border border-gray-200 bg-secondary/60 px-2 py-2.5 text-gray-300 focus:border-blue-300 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40 ">
                                            @foreach ($user->permissions as $existed)
                                                <option selected value="{{ $existed->id }}" selected>
                                                    {{ $existed->name }}</option>
                                            @endforeach
                                            @foreach ($permissions as $permission)
                                    ...
                                    <div class="flex justify-evenly">
                                        <div class="group relative w-fit">
                                            <button data-user-id="{{ $user->id }}"
                                                class="text-secondary update-usersRole">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    viewBox="0 0 24 24">
                                                    <path fill="currentColor"
                                                        d="M21.5 9h-5l1.86-1.86A7.99 7.99 0 0 0 12 4c-4.42 0-8 3.58-8 8c0 1.83.61 3.5 1.64 4.85c1.22-1.4 3.51-2.35 6.36-2.35s5.15.95 6.36 2.35A7.95 7.95 0 0 0 20 12h2c0 5.5-4.5 10-10 10S2 17.5 2 12S6.5 2 12 2c3.14 0 5.95 1.45 7.78 3.72L21.5 4zM12 7c1.66 0 3 1.34 3 3s-1.34 3-3 3s-3-1.34-3-3s1.34-3 3-3" />
...
@section('scripts')
    ...
    <script>
        ...
        document.querySelectorAll('.users_permissions').forEach(permissions => {
            var settings = {};
            new TomSelect(permissions, settings);
        });
        ...
        // var settings = {};
        // new TomSelect('#users_permissions', settings);
    </script>
    </script>
